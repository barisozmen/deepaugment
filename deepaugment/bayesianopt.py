...
EXPERIMENT_NAME = (
    f"{now.year}-{now.month:02}-{now.day:02}_{now.hour:02}-{now.minute:02}"
)
...
def calculate_reward(history):
    ...
    reward = history_df[history_df["acc_overfit"] <= 0.05]["val_acc"].nlargest(3).mean()
    return reward
...
def objective(
    ...
    augmented_data = augmenter.run(data["X_train"], data["y_train"], *trial_hyperparams)
...
def run_bayesianopt(
    ...
    if child_first_train_epochs > 0:
        ...
        notebook.record(
            0, ["first", 0.0, "first", 0.0, "first", 0.0, 0.0], 1, None, history
        )
    #
